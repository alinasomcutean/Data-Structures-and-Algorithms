...
int fprime(NodeT *Bucket[B], char *name)
{
    int sum = 0;
    int i;
    ...
    for(i = 0; i < 10; i++)
    {
        if(Bucket[(sum + i*i)%B] == NULL)
            return ((sum + i*i)%B);
    }
    return -1;
}

NodeT* fillnode(char *s)
{
...
void insert (NodeT* Bucket[B], char *s)
{
    NodeT* p=(NodeT*) malloc(sizeof(NodeT));
    int q;
    p=fillnode(s);
...
int findnode(NodeT* Bucket[B], char *s)
{
    int h=f(s);
    NodeT* p;
    p=Bucket[h];
...
void deletenode(NodeT *Bucket[B], char *s)
{
    int h = f(s);
    NodeT *p = NULL;
    p = Bucket[h];
