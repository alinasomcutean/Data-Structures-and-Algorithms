...
void insertAtLast(ListT** listPtr, int key)
{
...
{
    if(find(road, key) != NULL && road->first->truck_id == key)
    {
        insertAtLast(&garage, road->first->truck_id);
        deleteFirst(&road);
    }
    else
    {
...
void exitGarage(int key)
{
    ...
    {
        insertAtLast(&road, garage->first->truck_id);
        deleteFirst(&garage);
    }
    else
    {
        ...
        else if(garage->first->truck_id != key)
            printf("Error:%d is not first.", key);
    }
    printf("\n");
}
